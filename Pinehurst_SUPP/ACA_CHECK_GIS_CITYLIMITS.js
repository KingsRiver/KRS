...
var controlString = "X"; 			// Standard choice for control
...
try {
     ...
     parcelNum = par.getParcelNo();
     ParcelValidatedNumber = par.getParcelNo();
     //var cityLimits = getGISInfo2ASBUpdate("PINEHURST", "taxparcels", "TAXJURIS", parcelNum);
     var cityLimits = getGISInfo2ASB("PINEHURST", "taxparcels", "TAXJURIS", -1, "feet");
     var taxjuris = 'PH'

    if ((cityLimits != taxjuris)) {
        showMessage = true;
        cancel = true;
        comment("This property is outside of VOP jurisdiction. Please call 910-295-1900 for assistance.");
    } else {
        showMessage = false;
        cancel = false;
        comment('');
...
/------------------------------------------------------------------------------------------------------*/
function getGISInfo2ASBUpdate(svc,layer,attributename, parcelNum) {    
    var numDistance = 0
    ...
    if (bufferTargetResult.getSuccess())
    {
        ...
        //logDebug("Added " + attributename + " to buffer");
    }
    else
    ...
    if (gisObjResult.getSuccess())   {
        ...
        logDebug("Found gis obj: " + fGisObj);
    }
    else
    ...
    for (a1 in fGisObj) // for each GIS object on the Parcel.  We'll only send the last value
    {
        ...
        logDebug("Queried buffer " + fGisObj[a1]);
        if (bufchk.getSuccess()) {
            var proxArr = bufchk.getOutput();
            logDebug("Found proximity array: " + proxArr);
        }
        else
        ...
        for (a2 in proxArr)
        {
            ...
            logDebug("Have proxObj: " + proxObj);
            logDebug(proxObj.length);
            for (z1 in proxObj)
            {
                //var v = proxObj[z1].getAttributeValues()
                var queryObj = proxObj[z1];
                //explore(queryObj)
                var n = queryObj.getAttributeNames();
                for(var nameIndex in n) {
                    var val = n[nameIndex];
                    logDebug(val);
                }
                var v = queryObj.getAttributeValues();
                //logDebug(n + " " + v.length);
                retString = v[0];
                logDebug("Found value: " + v[0]);
            }
        }
    }
    return retString
